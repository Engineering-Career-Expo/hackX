...
function validateUser(user) {
  const schema = Joi.object().keys({
    name: Joi.string().min(3).max(50).required().error(errors => {
    errors.forEach(err => {
      switch (err.type) {
        case "any.empty":
    ...
    email: Joi.string().min(5).max(255).required().email().error(errors => {
    errors.forEach(err => {
      switch (err.type) {
        case "any.empty":
    ...
    message: Joi.string().min(25).max(500).required().error(errors => {
    errors.forEach(err => {
      switch (err.type) {
        case "any.empty":
